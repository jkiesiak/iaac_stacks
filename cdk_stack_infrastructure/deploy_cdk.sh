...
# Unique output directory for this environment
...
cdk synth --context env="$environment" --profile "$AWS_PROFILE" --output "$output_dir"

echo "---------- Step: Deploy the CDK stacks ----------"
cdk deploy --all --require-approval never --context env="$environment" --profile "$AWS_PROFILE" --output "$output_dir"
