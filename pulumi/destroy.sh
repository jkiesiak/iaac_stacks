...
# Select stack (fail if it doesn't exist, since there's nothing to destroy)
if ! pulumi stack select "$STACK_NAME"; then
  echo "Pulumi stack '$STACK_NAME' does not exist."
  exit 1
fi

# Destroy resources
pulumi destroy --yes --logtostderr --verbose=9

echo "✅ All resources in stack '$STACK_NAME' have been destroyed."
